...
module.options = require("antelope.buffers.options")
			...
			hooks = {
				on_enter = function(self)
					...
					if self.ctx.options.handle == "bufnr" then
						...
						notify(
							string.format("%s buffer%s deleted", count, count > 1 and "s" or ""),
							vim.log.levels.INFO
						)
						...
						repeat
							...
							if
								input == util.replace_termcodes(self.ctx.options.actions.delete)
								and #picker.entries > 1
							then
								return self:transition("OPEN")
							end
